"""Create (or reuse) a no-auth gateway for the Context7 MCP passthrough target.

Context7 is a public MCP server, so this tutorial uses a gateway with NO inbound
authorization (authorizerType=NONE) and forwards the caller's own Context7 API
key outbound via JWT passthrough. HTTP passthrough targets require a gateway with
no protocol type set, so the gateway is created directly via boto3 (the shared
GatewayBoto3Client.create_gateway hardcodes protocolType=MCP). Writes GATEWAY_ID
and GATEWAY_URL to the script-local .env.

> No-auth gateways are for public, rate-limited targets only. Do not use a
...
def main():
    ...
    if gateway_id:
        ...
        # authorizerType=NONE: no inbound authorization (Context7 is public).
