...
The gateway is shared with the runtime-agent and Claude Managed Agents labs, so
if it already exists this script reuses it. Writes GATEWAY_ID and GATEWAY_URL to
...
# Shared across the A2A, HTTP runtime-agent, Claude Managed Agents, and
# Databricks A2A labs: all attach their target to the same gateway, so this name
# must match across the deploy/cleanup scripts in each lab.
