"""Create a gateway (no protocol type) for an HTTP agentcoreRuntime target.

HTTP targets attach to a gateway that has no protocol type set, so this script
creates the gateway directly via boto3 with inbound CUSTOM_JWT auth. It writes
GATEWAY_ID and GATEWAY_URL to the script-local .env.
...
# Shared across the A2A and HTTP runtime-agent labs: both attach their target to
# the same gateway, so this name must match in both deploy/cleanup scripts.
...
def main():
    parser = argparse.ArgumentParser(
        description="Create a no-protocol-type gateway for HTTP runtime targets"
    )
    parser.add_argument(
        "--discovery-url",
        required=True,
    ...
    # The gateway is shared with the HTTP runtime-agent lab. Reuse it if it
    # already exists so the two labs attach targets to the same gateway.
    ...
    if gateway_id:
        ...
        # No protocolType key: required for http.agentcoreRuntime targets.
