...
This gateway (context7-gateway) is shared with the Context7 MCP lab. It uses NO
inbound authorization (authorizerType=NONE) and forwards the caller's own
Authorization header outbound via JWT passthrough. HTTP passthrough targets
...
# Shared with the Context7 MCP lab: both attach their passthrough target to this
# same no-auth gateway, so the name must match across both labs' scripts.
